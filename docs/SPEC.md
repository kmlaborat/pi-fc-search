...
| **D-037** | The extension resolves its `FASTCONTEXT_*` configuration (`loadFastContextConfig()`) **per `fc_search` call** inside `executeAgent` instead of snapshotting it once at module load | The module-load snapshot froze endpoint/model/key/sampling/timeout for the lifetime of a long-lived pi process: correcting a value in the package `.env` (or shell environment) required a full pi restart to take effect — inconsistent with D-028's principle that a fixed misconfiguration must not be replayed until process restart. `loadEnvFile()` still runs once at module init (D-012 precedence unchanged); per-call `loadFastContextConfig()` is pure `process.env` reads (no fs I/O), and the D-019/D-026 fail-fast checks now run per call as well. |
...
D-001…D-052, with D-011 superseded by D-012 and the D-019 `LLMAPIError`
