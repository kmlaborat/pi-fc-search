...
- **Defaults**: `path` defaults to `cwd`; `-C` (context) defaults to `3`, applied only
  when neither `-A` nor `-B` is given (D-035: explicit `-A`/`-B` define the context
  precisely and suppress the default, because rg applies the later context flag per
  direction and the pushed `-C 3` would otherwise override them); `-n` (line numbers)
...
The parsing rules live in the exported pure function `applyEnvContent()` (D-018:
only `FASTCONTEXT_*` keys are applied; D-012 precedence: `.env` values override
existing process environment entries). Two entry points share one code path:

```typescript
// src/fastcontext-agent/env.ts — implemented ONCE. loadEnvFile() is shared by
// extensions/index.ts and src/fastcontext-agent/llm.ts (both call it at module
// initialization) and stays once-guarded/idempotent; reloadEnvFile() (D-057)
// performs the file read on EVERY call and backs the /reload-fc-env command.

export function getEnvPath(): string;
// Package root only (env.ts lives in src/fastcontext-agent/ → two levels up)

export interface ReloadEnvResult {
  envPath: string;       // resolved package-root .env path
  found: boolean;        // whether the file exists
  appliedKeys: string[]; // FASTCONTEXT_* keys written into process.env
  ignoredKeys: string[]; // non-prefixed keys ignored per D-018
}

export function reloadEnvFile(): ReloadEnvResult {
  const envPath = getEnvPath();
  if (!existsSync(envPath)) return { envPath, found: false, appliedKeys: [], ignoredKeys: [] };
  // readFileSync in a try/catch — a missing or unreadable .env never throws;
  // it is reported in the result (and logged) so the command can warn.
  const content = readFileSync(envPath, 'utf-8');
  const ignoredKeys = applyEnvContent(content, process.env);
  // ... derive appliedKeys with the same parsing rules as applyEnvContent()
}
...
export function loadEnvFile(): void {
  if (loaded) return;
  loaded = true;
  reloadEnvFile(); // initial module-init load shares the runtime code path
}
```

**Runtime reload (D-057):** the `/reload-fc-env` command calls `reloadEnvFile()` and
re-applies the file's `FASTCONTEXT_*` keys into `process.env`; with D-037's
per-call config resolution, the edit takes effect on the next `fc_search` call
without a pi restart. Overwrite-only: keys removed from the `.env` are not
removed from `process.env`. The once-guarded module-init load and all parsing/
precedence rules above are unchanged.
...
| `FASTCONTEXT_TOP_P` | `0.95` | finite, 0–1 | Nucleus sampling probability (D-030). Was a code constant; now operator-configurable like the other v3 sampling settings. |
...
| `FASTCONTEXT_SEND_MAX_TOKENS` | `true` | `true`/`false` (case-insensitive) | Whether requests include `max_completion_tokens`. `false` omits the field for older OpenAI-compatible servers that reject it with a 400 (D-052). |
...
General-purpose tool calling model verified to cover fastcontext functionality without domain-specific training. See README §Model Selection for detailed analysis.
...
### KN-006: Deferred context-management candidates (not implemented)

The following were evaluated while fixing the 2026-08-18 context-bloat /
prefill-timeout incident (D-047/D-048) and deliberately **not** implemented.
They remain candidates for future context-management experiments:

- **Sliding Bisection** (kmlaborat/AnchorEdit, `v1/docs/SLIDING_BISECTION.md`):
  scope localization via overlapping 3-choice selections. Solved a different
  problem: it converts *vague positional intuition* into a precise scope for
  *repeated re-localization in an edit loop*. In fc_search, Grep localizes to
  line precision in one deterministic call at near-zero context cost, which
  strictly dominates bisection; the "develop positional awareness" phase is
  outside the algorithm and would cost a full read — the very thing to avoid.
- **Virtual File Partition Tree**: pre-partitioning large files into a
  navigable hierarchical structure. Superseded in scope by the simpler
  Grep-locate + `offset`/`limit` read pattern plus the D-047/D-048 budgets;
  revisit only if whole-file semantic skimming becomes a requirement.
- **Memory store + Recall tool**: keeping evicted tool results in an
  out-of-context store with a recall tool. Over-engineered while all three
  tools are deterministic pure functions whose outputs can be re-run for
  free (the repository is the source of truth); revisit if a
  non-reproducible tool (e.g. an external query) is ever added. The D-047
  stub format (tool_call_id + metadata) already anticipates this upgrade.
- **LLM compaction (summarization of old turns)**: rejected for the target
  model class — a 4B model's summary drops exactly the line-level detail the
  final answer (paths + line ranges) requires, the compaction call itself
  costs a cold prefill plus slow generation of the very context being
  shrunk, and it hurts reproducibility (§4.2 High Consistency).
...
| **D-004** | If the model's final message content is empty or whitespace-only, the agent surfaces an explicit `The LLM returned an empty response ...` message instead of an empty string | An empty final answer surfaced to the main agent as a mysterious "no response"; typically caused by output truncation at the max-token limit. *(The explicit message text is unchanged; the delivery mechanism is now a thrown `LLMAPIError` flagged `isError: true` — see D-021.)* |
...
| **D-017** | `Grep` rejects `head_limit` ≤ 0 with an actionable tool message instead of silently treating it as unspecified (default 100 lines) (§8.3); schema `minimum` changed from 0 to 1 | The port's `headLimit > 0` guard remapped `0` to the default 100 — reintroducing, for the degenerate value, exactly the silent-remapping failure mode D-010 removed for values ≥ 100: a model requesting 0 (most plausibly intending "no limit") received 100 lines with no explanation, and the old test even labeled the behavior "no limit" when it was "default limit". The explicit error names both valid options (omit for the default 100, or a positive value up to the 2000 cap) so the model self-corrects in one turn. "0 = unlimited" was considered and rejected: it would disable the only context-window safety cap of the sub-agent (D-010's 2000-line cap), contradicting the package's context-efficiency design target (SPEC §19). |
| **D-018** | The `.env` loader applies only `FASTCONTEXT_*` keys; every other `KEY=VALUE` line is ignored (with a `console.warn` listing the ignored keys) | The pre-D-018 loader wrote **any** key into `process.env` — and, per D-012, overrode the shell environment. A stray `.env` line such as `PATH=...`, `NODE_ENV=...`, or `RIPGREP_PATH=...` (the last one would silently redirect `getRgPath`, §10.1) would hijack the host pi process that hosts this extension. The package only ever reads `FASTCONTEXT_*` variables, so restricting the loader to that prefix preserves all documented configuration behavior (including D-012 precedence) while removing the escape hatch. The parsing rules (quotes, comments, `KEY=VALUE`) and the D-012 override precedence for `FASTCONTEXT_*` keys are unchanged; they are now implemented in the exported pure function `applyEnvContent()` for direct unit testing. |
| **D-019** | Fatal execution failures — missing `FASTCONTEXT_ENDPOINT`/`FASTCONTEXT_MODEL` (`ConfigurationError`), execution timeout (`TimeoutError`), and any other uncaught agent error — are thrown through the extension and returned as tool results with `isError: true`, instead of being returned as `"[ERROR] ..."` text with `isError: false`. The timeout and configuration messages themselves are unchanged. Cancellation mapping (`CancelledError` → "Search was cancelled", `isError: false`) is unchanged | An error that arrives as a *successful* tool result is indistinguishable from a (possibly empty) answer: the host model may reason over the error text as if it were search output, and pi's error surfacing/retry semantics keyed on `isError` never engage. The §6 error table already promises these cases are reported as errors. The config check also moved before the timeout timer is scheduled (a config error must not leave a pending timer), and the per-call `setTimeout`/abort listeners are now removed in a `finally` block on every path. The agent's `LLMAPIError`-as-assistant-message behavior (upstream parity) is intentionally unchanged. *(Superseded for the `LLMAPIError` case by D-021.)* |
| **D-020** | `Read` refuses files larger than 10 MB (`MAX_FILE_SIZE_BYTES`) and files whose first 8 KB contain a NUL byte; it additionally re-checks path containment on the `realpath`-resolved target; and the Read/Glob/Grep fallback resolution uses the tool's working directory (`resolve(ctx.cwd, path)`) instead of the Node process cwd | `readFile` loads the whole file into memory (in-memory paging), so a multi-GB artifact (minified bundle, log, dataset) would stall the host event loop and risk OOM; the size cap redirects the model to `Grep`, which streams. The NUL probe catches binary files before megabytes of garbage UTF-8 decoding. The lexical `isWithinCwd` check (§12) does not see through symlinks, so a symlink inside the working directory could read files outside it; the `realpath` re-check (both target and cwd, so symlinked cwd locations like macOS `/tmp` keep working) closes that hole. On `realpath` failure (file replaced mid-flight) the already-passed lexical check governs. The fallback-cwd fix is defensive: containment still gates every path, but `process.cwd()` gave confusing resolution results when pi runs from a different directory than the searched repo. The `Glob` tool's containment check also moved **before** its `existsSync`/`statSync` calls, which previously leaked existence information for out-of-cwd paths and was inconsistent with Read/Grep. The Read tool description states the new 10 MB/binary limits (description-accuracy, §19 C-7). |
| **D-021** | LLM API failures (`LLMAPIError`) and the empty final-response guard (D-004) are **thrown** through `Agent.run` and returned by the extension as tool results with `isError: true`, instead of being returned as a "successful" answer string (`LLM API call failed. So stopping the agent. …` / `[ERROR] The LLM returned an empty response …` with `isError: false`) | Supersedes the D-019 note that the `LLMAPIError`-as-assistant-message behavior was intentionally unchanged (upstream parity). The parity behavior had exactly the failure mode D-019 exists to prevent: an error arriving as a *successful* tool result is indistinguishable from an answer, the host model reasons over the error text as search output, and pi's `isError`-keyed surfacing/retry semantics never engage. The fake assistant message is no longer appended to the trajectory (the failed request is visible from the surrounding turns). The empty-response message text is unchanged (D-004); the extension already wraps thrown errors as `[ERROR] <message>`, so the host-visible string for the empty-response case is byte-identical to the old returned string. User cancellation mapping (`CancelledError`) is unchanged. |
| **D-022** | `Glob` and `Grep` re-check path containment on the `realpath`-resolved target (both target and cwd), mirroring the Read tool's D-020 defense | The lexical `isWithinCwd` check (§12) does not see through symlinks: Read was protected (D-020), but a symlink inside the working directory still allowed `Glob`/`Grep` to list/search files outside it via `rg --files`/`rg` on the link. Same rules as D-020: on `realpath` failure (path does not exist) the already-passed lexical check governs, and resolving both sides keeps symlinked cwd locations (e.g. macOS `/tmp`) working. The error message matches Read's form (`Permission error: <path> (resolves to <real>) …`). |
| **D-023** | `LLMClient.acall` retries transient failures: HTTP 408/429/500/502/503/504 and network-level `fetch` errors are retried up to 2 times (3 attempts total) with exponential backoff (default 500 ms × 2^n, configurable via the `retry_delay_ms` constructor option for tests). Retries are abort-aware (an aborted signal stops the loop and surfaces `CancelledError`/the raw abort as before); other 4xx and semantic `LLMAPIError`s (e.g. "No choices returned") are never retried | A single 5xx/429 or dropped connection ended the whole search — and, pre-D-021, surfaced as a "successful" error-text answer, so one transient blip wasted the user's query. Chat completions have no server-side side effects, so bounded retries are safe; the cap (3 attempts) bounds the extra latency against slow/CPU servers that already motivated the configurable global timeout (§19). The `Authorization` header is now omitted entirely when the API key is empty (local servers that ignore auth), since some servers reject an empty `Bearer ` header. |
| **D-024** | `Grep` truncates individual output lines longer than 2000 characters to 2000 characters with `...` appended (same per-line budget as `Read`, §8.1), before the line-count limit is applied | rg output lines are unbounded in length: a match inside a minified bundle or a log file produces a single megabyte-scale line that would land in the sub-agent's context verbatim and overflow the small models this package targets (§19). The `head_limit` cap is line-count-based and cannot bound this. All output modes are affected equally; normal-length output is byte-identical to before. |
| **D-025** | `Read` caps total output at 256 KB (`MAX_READ_OUTPUT_BYTES`): when the accumulated numbered lines exceed the budget the output stops, the header range reflects the lines actually shown, and a note (`... (output truncated at 262144 bytes; re-read with a larger offset and limit to continue)`) tells the model how to continue | The §8.1 limits (2000 lines × 2000 chars) still allow a ~4 MB worst-case result from a single default whole-file read, which would overflow the context window of the target small models and silently degrade the rest of the search. The cap is strictly above the output of any file the §8.1 limits alone would fully return for realistic line lengths, so existing behavior for normal files is unchanged. The tool description states the cap (§19 C-7 description accuracy). *(The 256 KB cap value is superseded by D-048 (64 KiB); the truncation mechanism is unchanged.)* |
| **D-026** | The extension validates `FASTCONTEXT_ENDPOINT` as an absolute http(s) URL (`validateEndpointUrl`) at the same fail-fast point as the missing-value check, throwing `ConfigurationError` for invalid values | An endpoint that is not an absolute URL (e.g. a missing scheme, `example.com/v1`) makes `fetch` throw a parse `TypeError` on every attempt, which the D-023 retry loop misclassifies as a transient network failure: two wasted retries and backoffs, then `LLM API call failed: Failed to parse URL`. Validating at startup turns the misconfiguration into an immediate, actionable `isError: true` result. |
| **D-027** | `LLMClient.acall` rewrites 400/413 responses whose body indicates a context-window exceedance (e.g. "context length", "too many tokens", "context_length_exceeded") into an actionable `LLMAPIError` ("The search conversation exceeded the model's context window. Re-run with a smaller max_turns or a more focused prompt.") | The agent history is unbounded by design, so long searches against the small models of §19 routinely outgrow the model's window; servers phrase the 400/413 body inconsistently (and small local servers often in the model's own idiolect), leaving the host agent with a raw body it cannot act on. Other 400/413 bodies are unchanged; neither status is retryable (D-023). |
| **D-028** | `getRgPath` caches only successful resolutions; a failed resolution (stale `RIPGREP_PATH`, transiently unavailable bundled module) is retried on the next call instead of being replayed | The pre-D-028 error cache made one failed resolution permanent for the process lifetime: after e.g. fixing a stale `RIPGREP_PATH`, every subsequent tool call replayed the cached error until pi restarted. Successes remain cached (the D-015 fail-fast for stale `RIPGREP_PATH` values is preserved for the duration of the resolution that observes the value). |
| **D-029** | A context-window exceedance is thrown as a typed `ContextWindowError` (extends `LLMAPIError`, D-027 message text unchanged) and `executeAgent` retries the search **once** with the turn budget halved (`max_turns = ceil(max_turns / 2)`) before surfacing the error. The retry is skipped when `max_turns < 4` (the reduced budget could not differ meaningfully) or when the controller was already aborted; if the retry also fails, the ORIGINAL overflow error is reported | The history is unbounded by design, so long searches against the small models of §19 routinely hit the D-027 failure mid-run. D-027's actionable message still requires the HOST agent to notice, re-invoke the tool, and pick a smaller `max_turns` — a round trip the extension can make itself. Halving the turn budget bounds the number of tool results accumulated, which is the documented mitigation. One retry only: a second overflow with a halved budget indicates per-turn output (not turn count) is the problem, and further retries would burn the global timeout. |
| **D-030** | `top_p` is operator-configurable via `FASTCONTEXT_TOP_P` (default `0.95`, finite 0–1, warn-and-fall-back on invalid values like the other §15.4 settings) and wired from `FC_CONFIG` through `RunFastContextAgentOptions.llm.topP` | `top_p` was the only sampling parameter left as a code constant after §19 made temperature/max-tokens/timeout configurable — an asymmetry that forced a code edit to tune it. The default is unchanged, so existing behavior is identical. |
| **D-031** | The Read tool's total-output budget (D-025, `MAX_READ_OUTPUT_BYTES`) is accumulated with `Buffer.byteLength(line, "utf8") + 1` instead of JS string length | The budget is advertised as **bytes** (tool description, truncation note, D-025), but JS string length counts UTF-16 code units: multi-byte content (CJK comments/strings) accumulated up to ~3x less "budget" than the bytes actually sent to the model, so the exact overflow protection D-025 exists for was weakest precisely where line content is widest. ASCII output is byte-identical to before. |
| **D-032** | The Read tool rejects non-integer `offset`/`limit` (`Read Tool: offset must be an integer.` / the existing `limit must be a positive integer.` message) **before** any filesystem access; `offset < 1` keeps the preserved §8.1 quirk (reads from line 1) and `limit <= 0` keeps its pre-existing error | The schema declares both parameters integers, but LLMs send non-integers: a fractional offset/limit previously produced fractional line numbers (`1.5|...`) in the output. Grep's `head_limit` has enforced its integer contract since D-017 — Read now does the same. Moving the validation ahead of the file I/O also avoids loading a 10 MB file only to reject the paging arguments. |
| **D-033** | `resolveDockerMountPath`'s cwd-basename comparisons (Strategy 2's first-component skip, Strategy 4's component match) are case-insensitive **only on win32**; on POSIX they are exact | The pre-D-033 `toLowerCase()` comparisons "corrected" case-mismatched components (e.g. `/PI-FC-SEARCH/...` → `./...`) on case-sensitive POSIX filesystems, where the mismatched name refers to a different, non-existent entry. Every strategy is `existsSync`-gated so the practical risk was low, but the correction then pointed at the wrong path's existence checks and produced misleading `Path corrected from ...` notes. Windows keeps the case-insensitive match because its default filesystems resolve the name either way. |
| **D-034** | `runRipgrep` stops accumulating stdout at `MAX_RG_STDOUT_BYTES` (16 MB) and drops all subsequent chunks | The line-length (D-024) and line-count (D-010) truncation in the Glob/Grep tools is applied to the *fully accumulated* stdout string, so a single pathological match (a megabyte-scale line in a minified bundle or log file) would sit in memory unbounded before any truncation ran. The retained 16 MB prefix vastly exceeds what the tools' line limits can ever display (Grep's maximum is 2000 lines x 2000 chars ~ 4 MB), so visible output is unchanged; the 10s spawn timeout still bounds the child's runtime. |
| **D-035** | `Grep`'s default `-C 3` context (§8.3) is a FALLBACK: it is applied only when neither `-A` nor `-B` is given. An explicit `-A` and/or `-B` reaches ripgrep unmodified and defines the context precisely (no default context on the other side; the model passes `-C` explicitly to get both sides) | The arg builder pushed `-B`/`-A` before `-C`, and ripgrep resolves overlapping context flags positionally (the later flag wins per direction) — so the default `-C 3` silently overrode every explicit `-B`/`-A` request: a model asking for `-A: 10` received the default 3/3 context with no way to know its request was dropped. Verified empirically against the bundled rg (`-B 5 -C 3` ≡ `-C 3`). The unspecified-default behavior (3/3) is unchanged; the schema `-C` description now states the fallback rule. Upstream-parity release (SPEC §19) makes fixing the collision permissible where the old `||`-era defaults were not. |
| **D-036** | `LLMClient.acall` treats a 2xx response whose body fails `JSON.parse` as a semantic `LLMAPIError` ("The LLM endpoint returned a response that is not valid JSON. …") that is **never retried** | The unguarded `response.json()` fell into the generic catch of the D-023 retry loop, where a malformed body was misclassified as a transient network failure: two wasted retries and backoffs against a deterministic server defect, then an opaque `LLM API call failed: Unexpected token < in JSON …` with no pointer at the endpoint. As an `LLMAPIError` it is re-thrown by the loop's own guard and surfaces immediately as an `isError: true` tool result naming the likely cause (wrong endpoint / non-OpenAI-compatible server). |
| **D-037** | The extension resolves its `FASTCONTEXT_*` configuration (`loadFastContextConfig()`) **per `fc_search` call** inside `executeAgent` instead of snapshotting it once at module load | The module-load snapshot froze endpoint/model/key/sampling/timeout for the lifetime of a long-lived pi process — inconsistent with D-028's principle that a fixed misconfiguration must not be replayed until process restart. Boundary note (clarified by D-054): `loadEnvFile()` still runs ONCE at module init (D-012 precedence unchanged), so a `.env`-FILE edit takes effect on the next pi start, not on the next call — UNLESS the operator uses `/reload-fc-env` (D-057), which re-reads the file into `process.env` at runtime and makes the edit effective on the next call; what the per-call `loadFastContextConfig()` re-reads is `process.env` itself (pure reads, no fs I/O) — which is what makes the D-019/D-026 fail-fast checks and the invalid-value warnings run per call. |
| **D-038** | `runRipgrep` stops accumulating stderr at `MAX_RG_STDOUT_BYTES` (16 MB, the D-034 constant) and drops all subsequent chunks | D-034 capped only stdout; stderr was still unbounded. A pathologically chatty rg run (e.g. permission-denied lines across a huge tree) would accumulate megabytes in memory before the reject, and the full stderr text becomes the tool result the sub-agent model reads. Retained 16 MB vastly exceeds any actionable rg error (usage errors and regex parse errors are short), so visible error output is unchanged. |
| **D-039** | `LLMClient.extractRawMessage` throws an explicit `LLMAPIError` ("No message returned from LLM API call. …") when the response's first choice has no `message` object; the error is never retried | Some servers return a 200 whose single choice carries only a `finish_reason` (partial/aborted completions). The unguarded spread `{ ...choice.message }` hit a `TypeError` deep in `acall`'s generic catch, where it was misclassified as a transient network failure: two wasted D-023 retries and backoffs against a deterministic server defect, then the opaque "LLM API call failed: Cannot convert undefined or null to object". As an `LLMAPIError` it is re-thrown by the loop's own guard and surfaces immediately as an `isError: true` tool result naming the likely cause. |
| **D-040** | `runRipgrep` accumulates stdout/stderr as `Buffer[]` and decodes each stream exactly once with `Buffer.concat(...).toString("utf8")` on settle; the D-034/D-038 byte caps are measured with `chunk.length` (real bytes) instead of string length | `stdout += chunk` relied on each pipe chunk's implicit per-chunk UTF-8 conversion, which corrupts a multi-byte character (e.g. CJK in a file name or match) split across a chunk boundary into U+FFFD. A single decode of the accumulated bytes cannot corrupt it. Measuring the caps in bytes instead of `string.length` also stops the advertised 16 MB cap from silently growing up to 4× for multi-byte content. |
| **D-041** | The D-029 context-window retry additionally requires at least 30 s of the total-execution timeout to remain (`MIN_RETRY_BUDGET_MS`); with less, the original `ContextWindowError` is reported without retrying | The retry runs under the SAME total-execution timeout as the first run (`FASTCONTEXT_TIMEOUT_SECONDS` bounds the whole execution), and an overflow is precisely the failure of a LONG run — most of the budget is usually already spent. Retrying with seconds to spare aborts mid-retry and surfaces a confusing `TimeoutError` instead of the actionable context-window message, defeating the purpose of the automatic mitigation. The other D-029 guards (not aborted, `maxTurns >= 4`, retry halved, original error reported on retry failure) are unchanged. |
| **D-042** | Exhausting the full turn budget (including the D-007 forced final turn) without a final answer throws the typed `NoFinalAnswerError` with the unchanged Requirement B message `` `No final answer after ${max_turns} turns.` `` instead of resolving that string | The string resolved as a normal return, so the extension returned it as a successful (`isError: false`) tool result — contradicting the D-019/D-021 principle that a failed search must be flaggable and letting the host agent reason over "No final answer after 15 turns." as if it were search output. Requirement B's mandated exit message text is preserved verbatim on the error. |
| **D-043** | `validateInput` rejects whitespace-only `description` and `prompt` (trim-length check) with the same "Missing or invalid …" messages as the empty-string case | A string of only spaces/newlines passed the `length === 0` check and burned the full turn budget (up to 50 LLM calls) and timeout budget on a search the sub-agent cannot possibly answer. The length maxima (100/2000) are still measured on the untrimmed string. |
| **D-044** | `Grep` honors explicit `-B: 0` / `-A: 0` (undefined checks in the arg builder, D-013 style) and rejects non-integer or negative `-B`/`-A`/`-C` values with an actionable tool message before spawning rg (§8.3) | The builder's truthiness checks (`if (rgArgs.beforeContext)`) silently dropped an explicit `0` — the same bug class D-013 removed for `-C` — so the request was not reflected in the rg command. Fractional or negative values would reach rg as-is and surface as opaque rg usage errors; the explicit validation (head_limit has enforced the same contract since D-017, Read paging since D-032) names the fix so the model self-corrects in one turn. `0` remains valid and means "no context on that side"; an explicit `-C: 0` now reaches rg as `-C 0` (behaviorally identical to before, which passed no context flag at all). |
| **D-045** | The integer `FASTCONTEXT_*` settings (`MAX_TOKENS`, `TIMEOUT_SECONDS`) accept only decimal integer literals; other notations (e.g. `1e3`, `0x10`, `12.0`) warn and fall back to the default like any invalid value (§15.4) | `parseNumber` used `Number()`, which parses exponent and hex notations: `FASTCONTEXT_TIMEOUT_SECONDS=1e3` was silently accepted as 1000 seconds (and `0x10` as 16) — values no operator would write down, accepted without warning and only range-checked. Restricting the literal to `/^\d+$/` matches what the documented settings are; the range validation and the warn-and-fall-back behavior are unchanged, and the floating-point settings (temperature, top_p) are untouched. |
| **D-046** | The `system.md` (SPEC §9.4 v3) `<final_answer>` example no longer shows a trailing parenthetical reason on a path line, and `fc_search`'s `use_citation` description (plus a README note) state that the final answer ALWAYS ends with a `<final_answer>` block in both modes — citation mode returns only the block, default mode returns the full final response (brief explanation plus the block) | The prompt rule mandates that the tag contain ONLY file paths with line ranges ("no prose, no explanations, no numbering"), but the example line showed a trailing "(Optional Brief Reason: …)" — a model imitating the example violated the rule it had just read, and in non-citation mode the host receives the full text with no way to know which instruction the model followed. The tool schema advertised the default mode as "full context with summaries" without mentioning the block at all, so host-side parsing of the output was undocumented. No other prompt text changed; the non-citation pass-through of the full final response (and `getFinalAnswer`'s behavior in citation mode) is unchanged. |
| **D-047** | Before EVERY LLM call, the combined tool-result payload is bounded: when the total UTF-8 byte size of all `role: "tool"` message `content` strings — the exact strings serialized into the API request; a **byte budget, not a token estimate** — exceeds `TOOL_RESULT_BUDGET_BYTES` (64 KiB, `context.ts`), tool results are replaced oldest-first with a short stub until the **post-stub** total (re-measured with stub sizes included) is ≤ budget. The stub preserves the message's `tool_call_id` and carries re-acquisition metadata (tool name + arguments looked up from the matching assistant `tool_call`, original size in bytes, first line of the original output, and a re-run/re-read instruction). The budget is **strict**: the most recent result is stubbed too when a single result alone exceeds it (only a pathological Grep result can; the D-048 Read cap makes a single Read unable). Full tool results are written to the trajectory at `context.add()` time and are never evicted from it; only the history entries sent to the LLM are mutated | 2026-08-18 incident: a batched whole-file Read of four files (one of them a 1193-line design doc, 98.6 KB of the total) grew the prompt to 43,384 tokens (164,028-byte request); the ~95 s cold prefill at ~458 tok/s exceeded the 120 s default total-execution timeout mid-run, and no existing recovery covered it — D-029 only fires on a server-side 400/413 *rejection*, while here the server accepted the prompt and simply took too long to prefill it. The history is unbounded by design (D-027/D-029), so the missing guard is a deterministic client-side byte budget on what is actually serialized to the LLM. Stubs (rather than LLM summarization) keep the mechanism deterministic, side-effect-free, and free of an extra LLM call; the read-only tools make re-acquisition cheap (re-run the tool, or Read with offset/limit — `--cache-prompt` servers prefill only the delta). The §11 round-trip invariant is unaffected: eviction only touches client-authored tool messages, never raw server messages. |
| **D-048** | The Read total-output cap (D-025 mechanism, unchanged) is reduced from 256 KB to **64 KiB** (`MAX_READ_OUTPUT_BYTES`); the Read tool description now states the 64 KiB cap and instructs Grep-first / offset-limit reads of large files (replacing the MS-model-era "speculatively read multiple files as a batch" encouragement); the system prompt gains three rules — do not read large files in full (Grep first, then offset/limit), note key file:line findings in the response text as the search proceeds (they survive D-047 eviction), and how to re-acquire a trimmed (stubbed) tool result | A 256 KiB single Read is ~64k tokens: against a ~458 tok/s prefill that is a ~140 s cold prefill — structurally beyond the 120 s default execution timeout (the incident's 43k-token / ~95 s prefill was exactly this path), and it dilutes the attention of the small models this package targets. 64 KiB keeps one read's cold prefill well under the timeout and, together with D-047's history budget, bounds the worst-case serialized request — verified by tests asserting every request stays ≤ 100 KiB, below the incident's 164,028-byte request (a regression bound against that observed failure, not a general performance guarantee). The truncation note, the 2000-line / 2000-char limits, and the 10 MB / binary refusals (D-020) are unchanged. |

| **D-049** | The opt-in real-server test suites (`tests/integration/real-server.test.ts`, `tests/integration/timeout-regression.test.ts`) skip themselves — with a logged reason — when the configured model cannot currently serve a completion, in addition to the existing credentials gate. A single minimal probe request per suite load (POST `/chat/completions`, `max_completion_tokens: 1`, 20 s timeout, `tests/utils/model-probe.ts`) returns true only on a 2xx; every other outcome (404 "no router for requested model", network failure, timeout) returns false. The probe never throws, so an unreachable endpoint can only SKIP these suites, never fail them | The opt-in suites were gated only on `FASTCONTEXT_API_KEY`/`FASTCONTEXT_ENDPOINT` being set, so a configured-but-unloaded model (or an unreachable endpoint) turned the test run RED for an environmental reason — a false signal that could mask or be mistaken for a real regression, and that made `npm test` unreliable on machines with a half-configured local server. `timeout-regression.test.ts` additionally now loads the package `.env` (`loadEnvFile()`) before the gate, matching the gate's documented "set in the environment OR the package .env" contract (the other suite already got this transitively via its top-level agent import). |
| **D-050** | The agent's abort signal (user cancellation / total-execution timeout) is threaded to tool execution: `ToolSet.callNormalized`/`call` accept an optional `AbortSignal` and forward it in `CallContext.signal`; `runRipgrep` accepts it and, on abort, kills the rg child immediately and rejects with the signal's reason (an already-aborted signal rejects before spawning). `Agent` passes its run signal to `callNormalized`; Grep/Glob pass `ctx.signal` to `runRipgrep` | Before D-050, cancelling a search (or firing the total-execution timeout) left an in-flight rg subprocess running out its own 10 s per-call timeout before the agent observed the abort at the next turn boundary or LLM call — the cancellation was delayed by up to 10 s per tool call, and on Windows a killed-but-unobserved child also keeps its stdout/stderr pipes (and thus the event loop) alive. The 10 s per-call timeout, the sequential execution, and the error-isolation behavior are all unchanged; the signal is an ADDITIONAL stop path. Tools that do not spawn subprocesses (Read) ignore the signal. |
| **D-051** | The eviction loop in `evictToolResults` (`context.ts`, D-047) maintains a RUNNING byte total — updated incrementally by the exact stub/original byte delta of each replacement — instead of re-measuring the whole history array on every iteration; the termination condition and all D-047 guarantees (oldest-first, post-stub sizes, strict budget, idempotency, report shape) are arithmetically unchanged | The pre-D-051 loop called `measure()` (an O(n) scan over the full history) at the top of each of up to n stub replacements — O(n²) in the eviction case. This runs before EVERY LLM call (D-047), so a long search that evicts dozens of results paid the quadratic cost on every turn. The incremental total is byte-identical to the old post-stub re-measurement (same `Buffer.byteLength` inputs), so `bytesBefore`/`bytesAfter`/`evictedCount` are unchanged. |
| **D-052** | `max_completion_tokens` is now optional in LLM requests: the new `FASTCONTEXT_SEND_MAX_TOKENS` setting (`true`/`false`, case-insensitive, default `true`, warn-and-fall-back on invalid values like the other §15.4 settings) flows through `loadFastContextConfig()` → `RunFastContextAgentOptions.llm.sendMaxTokens` → `LLMClient` (`send_max_tokens`); when `false`, the field is omitted from the payload entirely (the server's own default applies) | The payload sent `max_completion_tokens` unconditionally. Newer OpenAI-compatible stacks (OpenAI ≥2024, llama.cpp, vLLM) accept it, but older/vendored servers 400 on the unknown field — and since the 400 is not in the D-023 retryable set, the whole search died on every call with the raw server body. Default `true` keeps the long-standing payload shape (and the D-048 prefill budget, which relies on the cap being enforced server-side); `false` trades that enforcement for compatibility with servers that reject the field. |
| **D-053** | `runRipgrep` (`tools/rg.ts`) truncates the slice of stderr that goes into the reject's error message at 8 KiB (UTF-8-safe cut: a code point split at the byte cut is dropped, not left as a trailing U+FFFD), appending a `[stderr truncated at 8192 bytes]` note. The 16 MB stdout/stderr ACCUMULATION caps (D-034/D-038) are unchanged — only the error message is bounded | The rejected error text becomes the failed tool result's text. The D-047 tool-result budget (64 KiB) stubs any such oversized result before the next LLM call, so the sub-agent model would only ever see the stub's first line — surfacing megabytes of stderr as an error message bloated the trajectory without improving what the model reads. Actionable rg stderr (usage errors, regex parse errors) fits far below the cap. |
| **D-054** | *(documentation fix, no behavior change)* The README's D-012 note and the D-037 rationale claimed a corrected package `.env` value "takes effect on the next `fc_search` call (no pi restart needed)". The code has always been the other way: `loadEnvFile()` is the idempotent loader that runs ONCE at module initialization, so a `.env`-FILE edit takes effect on the next pi start; D-037's per-call resolution re-reads `process.env` (which is what makes the D-019/D-026 fail-fast checks and the invalid-value warnings run per call). The docs are corrected to state this | The false "no restart needed" claim is a real operator trap: someone editing `.env` expecting the next search to pick it up would get stale configuration and burn time diagnosing a phantom problem. The README's Configuration section already stated the accurate "read once / restart pi" semantics — the D-012 note now agrees with it. |
| **D-055** | Code hygiene: (a) `runRipgrep` declares `settled` / `timeoutHandle` / `onAbort` / `clearAbortListener` all BEFORE the handlers that use them (previously the `setTimeout` callback referenced `settled` and `clearAbortListener` that were declared after it — safe only because the callbacks run asynchronously); (b) `loadSystemPrompt` (`prompt.ts`) is now async (`readFile`/`readdir` from `fs/promises`) and `Agent` takes the pre-loaded `systemPrompt` as a constructor argument — `runFastContextAgent` awaits the load before constructing the agent | (a) a TDZ-style readability trap that invites a "refactor" into a real temporal-dead-zone bug; (b) the synchronous `readdirSync` of the SEARCHED working directory was the last synchronous fs call left in the agent — on a slow or networked filesystem it blocked the host (pi) event loop for every search, the exact failure class the Read/Glob tools' sync→async review fixes removed. The `${WORK_DIR}`/`${WORK_DIR_LS}` substitution and the rendered prompt are byte-identical; only the loading mechanism and the `Agent` constructor signature changed. |
| **D-056** | *(documentation fix, no behavior change)* README and SKILL.md examples now show ABSOLUTE paths in the `<final_answer>` examples (matching the system-prompt contract, which mandates "Every path you pass to a tool must be an absolute path" and the example block in `system.md`), and SKILL.md no longer describes `FASTCONTEXT_ENDPOINT` as "the fastcontext endpoint" (v3 is model/endpoint-agnostic — any OpenAI-compatible endpoint, README §Model Selection) | The relative-path examples contradicted the output contract the sub-agent actually produces (an operator parsing `fc_search` output for paths would get absolute paths, not the documented-looking relative ones), and the stale "fastcontext endpoint" wording contradicted the v3 rename (the README already explains the `FASTCONTEXT_*` names are kept for upstream continuity only). |
| **D-057** | A new `/reload-fc-env` extension command re-reads the package `.env` at runtime: `env.ts` gains `getEnvPath()` and a re-runnable `reloadEnvFile(): ReloadEnvResult` (performs the file read on EVERY call, unlike the once-guarded `loadEnvFile()`, which now delegates to it with its once-guard intact). The command re-applies the file's `FASTCONTEXT_*` keys into `process.env` and reports the result via `ctx.ui.notify` (applied keys + currently effective endpoint/model/timeout; warnings for a missing `.env` or ignored non-prefixed keys). Combined with D-037's per-call `loadFastContextConfig()`, a `.env` edit takes effect on the NEXT `fc_search` call without a pi restart. Semantics preserved: `FASTCONTEXT_*`-only application (D-018), `.env`-overrides-shell precedence (D-012), missing/unreadable `.env` never throws (§15.2 policy). Deliberate limitation: keys REMOVED from the `.env` are not removed from `process.env` (overwrite-only, matching `applyEnvContent()`'s contract) | D-054 corrected the docs to state that a `.env`-FILE edit takes effect on the next pi start — accurate for the default flow, but it left operators with no way to pick up a corrected endpoint/model mid-session (e.g. after pointing the package at a different local server), which is exactly the stale-configuration trap D-054 was written about. The command deliberately does NOT use `ctx.reload()` (a full extension reload): config resolution is per-call from `process.env` (D-037), so re-applying the file's keys is sufficient, and a runtime reload would churn the session unnecessarily. It also adds no dependency (§2.2.4: Node built-ins only) and touches none of the §4.7 external contract (`fc_search` name/schema/return shape unchanged). D-054's "read once / restart pi" statement remains correct as the DEFAULT behavior when `/reload-fc-env` is not used; the README now documents both paths. |
...
D-001…D-057, with D-011 superseded by D-012 and the D-019 `LLMAPIError`
note superseded by D-021) remains in force unchanged.
...
| C-7 | **Tool description accuracy pass** (behavior unchanged except D-013) | `Read` `offset` field description: removed the claim that "negative values count backwards from the end" — the implemented behavior (frozen by §8.1 and the preserved-quirks list) treats `offset < 1` as `1`; the description now states that. `Grep` `head_limit` field description: "When unspecified, shows all results from ripgrep" replaced with "the first 100 lines are shown" — the default cap of 100 for unspecified `head_limit` is frozen by §8.3/D-010. |
| C-8 | **Tool-result history budget** (D-047) | A 64 KiB byte budget on the combined size of all tool-result contents (the strings serialized into the LLM request) is enforced before every LLM call: oldest results are stubbed (with re-acquisition metadata) until the post-stub total fits. The trajectory keeps the full originals. |
| C-9 | **Read cap + prompt guardrails** (D-048) | Read output cap reduced 256 KB → 64 KiB (single-call constraint, separate from C-8's history budget); system prompt and Read description updated: large files are read Grep-first with offset/limit, key file:line findings are noted in the response text (survives eviction), and trimmed results may be re-acquired. |
