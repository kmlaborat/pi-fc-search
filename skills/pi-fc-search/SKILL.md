...
The skill automatically loads `.env` from the package directory (`pi-fc-search/.env`) at module initialization time. After editing `.env`, run `/reload-fc-env` inside a running pi session to apply the changes to the next `fc_search` call without restarting pi (SPEC §18/D-057).
